...
/**
 * Returns real type of input.
 * 
 * Example:
 *
 *     type.of( Number );     // Because it's the constructor.
 *     //=> 'function'        //
 *
 *     type.of( new Number(3) );
 *     //=> 'number'
 *
 *     type.of( NaN );
 *     //=> 'NaN'
...
type.of = item => {
  // Typof 'number' can be real number ot NaN.
  const numberOrNaN = i => Number.isNaN( i )
    ? 'NaN'
    : 'number';

  // Detects real type for items,
  // that have typeof 'object'.
  const detectObjectType = i => { 
    switch ( true ) {
      case item instanceof Number:
        return numberOrNaN( i );
  ...
  switch ( typeof item ) {
    case 'object':
      return detectObjectType( item );
    case 'number':
      return numberOrNaN( item );
    default:
      return typeof item;
  }
};
